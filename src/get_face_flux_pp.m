function face_flux_pp = get_face_flux_pp(fieldM, fieldP, param)
...
% Compute flux between adjacent poroelastic elements
...
e11M  = fieldM(param.pp_face_node,1);
e22M  = fieldM(param.pp_face_node,2);
e33M  = fieldM(param.pp_face_node,3);
e12M  = fieldM(param.pp_face_node,4);
e23M  = fieldM(param.pp_face_node,5);
e13M  = fieldM(param.pp_face_node,6);
zetaM = fieldM(param.pp_face_node,7);
...
e11P  = fieldP(param.pp_face_node,1);
e22P  = fieldP(param.pp_face_node,2);
e33P  = fieldP(param.pp_face_node,3);
e12P  = fieldP(param.pp_face_node,4);
e23P  = fieldP(param.pp_face_node,5);
e13P  = fieldP(param.pp_face_node,6);
zetaP = fieldP(param.pp_face_node,7);
...
face_flux_pp(:,1) = tmpS.*onx.*(param.cs_i_ppP.*nnTnx + param.mu_fr_ppP.*nndvx);
face_flux_pp(:,2) = tmpS.*ony.*(param.cs_i_ppP.*nnTny + param.mu_fr_ppP.*nndvy);
face_flux_pp(:,3) = tmpS.*onz.*(param.cs_i_ppP.*nnTnz + param.mu_fr_ppP.*nndvz);
face_flux_pp(:,4) = 0.5*tmpS.*(...
    param.cs_i_ppP .*(ony.*nnTnx + onx.*nnTny) +...
    param.mu_fr_ppP.*(ony.*nndvx + onx.*nndvy) ...
    );
face_flux_pp(:,5) = 0.5*tmpS.*(...
    param.cs_i_ppP .*(onz.*nnTny + ony.*nnTnz) +...
    param.mu_fr_ppP.*(onz.*nndvy + ony.*nndvz) ...
    );
face_flux_pp(:,6) = 0.5*tmpS.*(...
...
for fld1 = 1:param.Nfields
    face_flux_pp(:,fld1) = face_flux_pp(:,fld1) + tmp_fastP.*param.r1_p_pp(:, fld1);
end
...
for fld1 = 1:param.Nfields
    face_flux_pp(:,fld1) = face_flux_pp(:,fld1) + tmp_slowP.*param.r4_p_pp(:, fld1);
end